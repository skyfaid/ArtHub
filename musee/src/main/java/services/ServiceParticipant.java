...
public class ServiceParticipant implements ServiceCrud<Participant> {
    ...
    public List<Participant> afficher() {
        ...
        String req = "SELECT p.*, u.nom AS user_nom, u.prenom AS user_prenom, e.nom AS event_nom " +
                "FROM participants p " +
        ...
        try {
            ...
            while (rs.next()) {
                // Fetch nom and prenom from the resultSet
                ...
                listParticipant.add(new Participant(
                        ...
                        nom,
                        prenom,
                        rs.getString("event_nom"))); // Include event name in Participant object
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
