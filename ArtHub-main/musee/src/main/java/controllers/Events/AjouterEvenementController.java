...
public class AjouterEvenementController {
    ...
    public void ajouterNouveauEvenement(ActionEvent event) {
        ...
        if (dateDebut.isBefore(LocalDate.now())) {
            Notification notification = new Notification("Start date must be today's date or a future date.", notificationContainer);
            notification.show();
            return;
        }
        ...
        try {
            ...
            // Validate dateDebut is today or in the future
            if (dateDebut.isBefore(LocalDate.now())) {
                Notification notification = new Notification("La date de début doit être celle d'aujourd'hui ou une date future.", notificationContainer);
                notification.show();
                return;
            }
           ...
           //Evenement newEvent = new Evenement(nomText, dateDebut, dateFin, lieuText, typeText, descriptionText, nombrePlacesInt, imageURL);
            ...
            Notification notification = new Notification("Date debut doit être avant date fin", notificationContainer);
            notification.show();
        }
    }
