...
public class ServiceEvenement implements ServiceCrud<Evenement> {
    ...
    public void ajouter(Evenement e) {
        // Updated SQL query to include the nombreParticipants column and set it to 0 by default.
        String req = "INSERT INTO evenements(nom, datedebut, datefin, lieu, type, description, nombrePlaces, nombreParticipants, posterUrl, videoUrl) VALUES(?, ?, ?, ?, ?, ?, ?, 0, ?, ?)";
        try {
            PreparedStatement st = cnx.prepareStatement(req);
            st.setString(1, e.getNom());
            st.setDate(2, java.sql.Date.valueOf(e.getDatedebut())); // Convert LocalDate to java.sql.Date for datedebut
            ...
            st.setString(6, e.getDescription());
            st.setInt(7, e.getNombrePlaces());
            // No need to set nombreParticipants as it is set to 0 in the SQL query
            st.setString(8, e.getPosterUrl()); // Set the poster URL
            st.setString(9, e.getVideoUrl()); // Set the video URL
            st.executeUpdate();
            System.out.println("Event added successfully!");
   ...
   @Override
   public void modifier(Evenement e) {
       // Update SQL statement to include datedebut and datefin
       String req = "UPDATE evenements SET nom = ?, datedebut = ?, datefin = ?, lieu = ?, type = ?, description = ?, nombrePlaces = ?, posterUrl = ?, videoUrl = ? WHERE id = ?";
       try {
           PreparedStatement st = cnx.prepareStatement(req);
           st.setString(1, e.getNom());
           st.setDate(2, java.sql.Date.valueOf(e.getDatedebut())); // Set datedebut by converting LocalDate to java.sql.Date
           st.setDate(3, java.sql.Date.valueOf(e.getDatefin())); // Set datefin by converting LocalDate to java.sql.Date
           st.setString(4, e.getLieu());
           st.setString(5, e.getType());
           st.setString(6, e.getDescription());
           st.setInt(7, e.getNombrePlaces());
           st.setString(8, e.getPosterUrl()); // Set the poster URL
           st.setString(9, e.getVideoUrl()); // Set the video URL
           st.setInt(10, e.getId()); // Ensure the id is set correctly for the WHERE clause
           st.executeUpdate();
           System.out.println("Event modified successfully");
       } catch (SQLException ex) {
           System.out.println(ex.getMessage());
